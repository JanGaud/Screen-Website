...
<section class="relative mb-10 lg:mb-20 text-center md:text-left flex justify-center" data-slice-type={slice.slice_type} data-slice-variation={slice.variation}>
    ...
    <div class="z-20 w-full flex flex-wrap flex-col items-center gap-4 cursor-none pointer-events-none">
        <h2 bind:this={titleAnimation} class="font-bold tracking-tighter mb-4 text-4xl lg:text-6xl drop-shadow-lg w-fit text-center border-b-8 border-saffron">
            {slice.primary.title}
        </h2>
        <div class="flex flex-wrap justify-center items-center gap-6 lg:grid lg:grid-cols-2">
            <div class="h-[300px] w-full md:h-full">
				...
				src={slice.primary.glb_link} alt="3D avatar" ar shadow-intensity="2" camera-controls auto-rotate></model-viewer>
            </div>
